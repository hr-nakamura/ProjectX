...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="全_要員数">
    ...
    <table class="table" border="0" CELLPADDING="0" CELLSPACING="0">
      ...
      <tbody>
        <xsl:call-template name="全_グループ">
          <xsl:with-param name="element" select="本部[@name=$本部名]/グループ"/>
  ...
  <xsl:template name="全_valueOut_Loop">
    ...
    <xsl:choose>
      <xsl:when test="$m &lt; $max">
        <xsl:variable name="sumVal" select="sum($element/保有リソース[@name=$stat]/要員数/項目[@name='社員' or @name='パート' or @name='協力' or @name='社内発注']/月[@m=$m])"/>
        <td class="groupTypeData">
          ...
          <xsl:call-template name="value_Out">
            <xsl:with-param name="color" select="$element/保有リソース[@name=$stat]/月情報/月[@m=$m]"/>
            <xsl:with-param name="unit" select="$unit"/>
            <xsl:with-param name="form" select="$form"/>
      ...
      <xsl:otherwise>
        <td class="groupTypeData">
          <xsl:variable name="sumVal_All" select="sum($element/保有リソース[@name=$stat]/要員数/項目[@name='社員' or @name='パート' or @name='協力' or @name='社内発注']/月)"/>
          <xsl:call-template name="value_Out">
