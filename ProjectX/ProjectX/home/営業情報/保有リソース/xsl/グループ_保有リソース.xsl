...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="valueOut_Loop">
    ...
    <xsl:choose>
      <xsl:when test="$m &lt; $max">
        <xsl:variable name="sumVal" select="sum(保有リソース[@name=$stat]/要員数/項目[@name=$mode]/月[@m=$m])"/>
        <td class="groupTypeData">
          ...
          <xsl:call-template name="value_Out">
            <xsl:with-param name="color" select="保有リソース[@name=$stat]/月情報/月[@m=$m]"/>
            <xsl:with-param name="unit" select="$unit"/>
            <xsl:with-param name="form" select="$form"/>
      ...
      <xsl:otherwise>
        <td class="groupTypeData">
          <xsl:variable name="sumVal_All" select="sum(保有リソース[@name=$stat]/要員数/項目[@name=$mode]/月)"/>
          <xsl:call-template name="value_Out">
  ...
  <xsl:include href="グループ_保有リソース_合計.xsl"/>
