  ...
  <xsl:template match="/">
    ...
    <xsl:if test="count(本部[@name=$本部名]/グループ) > 0">
      ...
      <xsl:call-template name="実工数">
        <xsl:with-param name="本部名" select="$本部名"/>
        <xsl:with-param name="stat" select="$本部名"/>
        <xsl:with-param name="mode" select="'グループ'"/>
        <xsl:with-param name="form" select="'#,##0.00'"/>
      </xsl:call-template>
      ...
      <xsl:call-template name="実工数">
        <xsl:with-param name="本部名" select="$本部名"/>
        <xsl:with-param name="stat" select="$本部名"/>
        <xsl:with-param name="mode" select="'間接'"/>
        <xsl:with-param name="form" select="'#,##0.00'"/>
      </xsl:call-template>
  ...
  <xsl:template name="valueOut_Loop">
    ...
    <xsl:choose>
      <xsl:when test="$m &lt; $max">
        ...
        <xsl:variable name="sumVal" select="sum(実工数/データ/項目[@name=$mode]/月[@m=$m])"/>
      ...
      <xsl:otherwise>
        <td class="groupTypeData">
          ...
          <xsl:variable name="sumVal_All" select="sum(実工数/データ/項目[@name=$mode]/月)"/>
          <xsl:call-template name="value_Out">
            <xsl:with-param name="form" select="$form"/>
  ...
  <xsl:template name="valueOut_Loop_All">
    ...
    <xsl:choose>
      <xsl:when test="$m &lt; $max">
        ...
        <xsl:variable name="sumVal" select="sum($element/グループ/実工数/データ/項目[@name=$mode]/月[@m=$m])"/>
        <td class="groupTypeData">
          <!--<xsl:attribute name="style">
            <xsl:if test="$sumVal &lt; 0">
              <xsl:value-of select="'color:tomato'"/>
      ...
      <xsl:otherwise>
        <td class="groupTypeData">
          ...
          <xsl:variable name="sumVal_All" select="sum($element/グループ/実工数/データ/項目[@name=$mode]/月)"/>
          <xsl:call-template name="value_Out_All">
            <xsl:with-param name="form" select="$form"/>
            <xsl:with-param name="value" select="$sumVal_All"/>
          </xsl:call-template>
