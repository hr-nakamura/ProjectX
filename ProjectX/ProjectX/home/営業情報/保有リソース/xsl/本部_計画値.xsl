...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    ...
    <xsl:if test="count(本部/グループ) > 0">
      ...
      <xsl:call-template name="計画値">
        <xsl:with-param name="stat" select="'計画値'"/>
  ...
  <xsl:template name="valueOut_Loop">
    ...
    <xsl:choose>
      <xsl:when test="$m &lt; $max">
        <xsl:variable name="sumVal" select="sum(グループ/計画値/データ/項目[@name=$mode]/月[@m=$m])"/>
      ...
      <xsl:otherwise>
        <td class="groupTypeData">
          <xsl:variable name="sumVal_All" select="sum(グループ/計画値/データ/項目[@name=$mode]/月)"/>
          <xsl:call-template name="value_Out">
